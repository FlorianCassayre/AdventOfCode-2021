...
@main def Day09 = Day(9) { (input, part) =>
  ...
  extension (m: map.type)
    def apply(p: Position): Int = p.pipe((i, j) => m(i)(j))
    def has(p: Position): Boolean = p.pipe((i, j) => m.indices.contains(i) && m(i).indices.contains(j))
  ...
  val sources =
    for
      ...
      j <- map(i).indices
      p = (i, j)
      if map.adjacentOf(p).forall(q => map(p) < map(q))
    yield p
  ...
  def bfs(visit: Set[Position], seen: Set[Position]): Set[Position] =
    if visit.nonEmpty then
      val next = visit.flatMap(p => map.adjacentOf(p).filter(map(_) < 9).filterNot(seen.contains))
      bfs(next, seen ++ next)
    else
      seen
